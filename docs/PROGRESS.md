...
## 8. 新增优化功能（v1.1，不发布 release）

### 8.1 画布与状态栏

- [x] 画布右下角/状态栏实时显示鼠标像素坐标 (x, y)
- [x] 画布右下角/状态栏实时显示当前缩放百分比
- [x] 画笔/橡皮擦显示圆形光标预览圈
- [x] 画布重构成图层架构，棋盘格仅作为图片底部图层

### 8.2 交互式裁剪

- [x] 裁剪工具支持画布上拖拽边缘/角点调整裁剪区域
- [x] 裁剪时按住 Shift 保持原始比例
- [x] 裁剪面板与画布交互实时双向同步

### 8.3 精灵图可视化编辑器

- [x] 精灵图编辑器作为独立窗口打开，点击工具栏“精灵图”时隐藏默认主窗口
- [x] 支持批量上传多张精灵图片，底部画廊显示所有缩略图
- [x] 可自定义画布宽高、行数/列数，自动生成固定网格画布
- [x] 拖拽画廊中的精灵图片到网格单元格进行填充
- [x] 点击导出即可生成完整精灵图 PNG
- [x] 修复导出时 `memoryview` 对象没有 `setsize` 的报错
- [x] 每个格子显示中心辅助线，用于对齐检查
- [x] 支持一键将所有精灵居中到所在格子
...
- [x] 工具栏新增矩形选择工具
- [x] 工具栏新增自由选择（套索）工具
- [x] 支持 Ctrl+C 复制选区，Ctrl+V 粘贴为独立新图层
- [x] 工具栏新增移动工具用于调整图层位置
